...
const DEX_HIPPO: u8 = 1;
    ...
    public entry fun one_step_route<X, Y, E>(
        ...
        let coin_in = coin::withdraw<X>(from_add, x_in);
        ...
        check_and_deposit_opt(from_add, coin_remain_opt);
        check_and_deposit_to_address(to_sender, coin_out);
    }
    ...
    >(
        ...
        check_and_deposit_opt(from_add, coin_x_remain);
        check_and_deposit_opt(from_add, coin_y_remain);
        check_and_deposit_to_address(to_sender, coin_z);
    }
    ...
    >(
        ...
        check_and_deposit_opt(from_add, coin_x_remain);
        check_and_deposit_opt(from_add, coin_y_remain);
        check_and_deposit_opt(from_add, coin_z_remain);
        check_and_deposit_to_address(to_sender, coin_m);
    }
    ...
    fun addCoinFee<X>(coin: &mut coin::Coin<X>,fee_bips: u8) acquires ModuleData{
        let fee_value = coin::value(coin) * (fee_bips as u64) / 10000;
    ...
    public entry fun swap_with_fixed_output<X, Y, E>(
        ...
        let coin_in = coin::withdraw<X>(from_add, max_in);
        ...
        addCoinFee(&mut coin_in,fee_bips);
        assert!(coin::value(&coin_in) > 0, E_OUTPUT_NOT_EQAULS_REQUEST);

        check_and_deposit(from_add, coin_in);
        check_and_deposit_to_address(to_sender, coin_out);
    }
...
// fun check_and_deposit<X>(sender: &signer, coin: coin::Coin<X>) acquires ModuleData{
        // let sender_addr = signer::address_of(sender);
        // let b_store = borrow_global_mut<ModuleData>(@swap_account);
        // let signer_r = account::create_signer_with_capability(&b_store.signerCapability);
        ...
        // let coins_fee = coin::extract(&mut coin, (amount_out*3)/1000);//have to add fee 0.3
        // coin::deposit(signer::address_of(&signer_r), coins_fee);
        // coin::deposit(sender_addr, coin)
    // }